...
export const postModules = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const putModule = async (req: Request, res: Response): Promise<void> => {
  ...
  if (req.params.moduleId) {
    let module = await ModuleModel.findByIdAndUpdate({
      hidden: false,
      _id: req.params.moduleId
    }, req.body);
    if (module) {
    ...
    res.status(404).type('json').send({ error: 'No moduleId given in parameters' });
  }
};
